...
public class PaymentDAO {
	public boolean insertPayment(PaymentDTO dto) {
		String sql = "INSERT INTO payment (u_no, c_no, h_no, pay_amount, pay_card, pay_name, pay_phone, pay_address, pay_check) "
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
		...
		try (Connection conn = DBManager.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
			...
			pstmt.setInt(9, dto.getPay_check());
	...
	public boolean addPayment(PaymentDTO dto) {
		String sql = "INSERT INTO payment "
				+ "(u_no, c_no, h_no, pay_amount, pay_card, pay_date, pay_name, pay_phone, pay_address, pay_check) "
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		try (Connection conn = DBManager.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql);) {
			pstmt.setInt(1, dto.getU_no());
			pstmt.setInt(2, dto.getC_no());
